...
export default function SplashScreen() {
  ...
  useEffect(() => {
    if (minTimeElapsed && !loading && hasSeenOnboarding !== null) {
      if (session) {
        router.replace('/(tabs)');
  ...
  }, [minTimeElapsed, loading, session, hasSeenOnboarding]);
...
const styles = StyleSheet.create({
  ...
  logoGroup: { flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 },
  icon: { marginTop: 240 },
